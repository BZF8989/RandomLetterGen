...
public class RandomLettersGenerator {
    ...
    public static char singleLowerCaseGenerator(char start, char end) {
        ...
        if (start > end || start < 'a' || end > 'z')
            ...
            return LOWERCASE.charAt(start + r.nextInt(end - start + 1) - 'a');
    }
    ...
    public static char singleUpperCaseGenerator(char start, char end) {
        ...
        if (start > end || start < 'A' || end > 'Z')
            ...
            return UPPERCASE.charAt(start + r.nextInt(end - start + 1) - 'A');
    }
     ...
     */
    public static String numberOfAnyCaseLetters(int n) {
        StringBuilder s = new StringBuilder();
        for (int i = 0; i < n; i++) {
     ...
     */
    public static String numberOfAnyCaseLetters(char start, char end, int n) {
        if (start > end || start < 'A' || end > 'z')
            return null;
        StringBuilder s = new StringBuilder();
        for (int i = 0; i < n; i++) {
